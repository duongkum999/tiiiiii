...
const login = require("fca-unofficial-tik");
let appStateFile;
...
function onBot({ models }) {
  try { var appState = JSON.parse(require('fs').readFileSync(appStateFile, 'utf8')) } catch { var appState = require('fs').readFileSync(appStateFile, 'utf8').split(/;/).map(($, i, o, u = $.split(/=/))=> ({
    key: u[0].trim(), value: u[1], domain: 'facebook.com', path: '/', expires: new Date(Date.now()+(1000*60*60*24*10)).toUTCString(),
})).filter($=>'datr,m_pixel_ratio,c_user,i_user,xs,m_page_voice,sb,fr,presence'.split(',').includes($.key)) };
